...
export interface EditorProps {
   ...
   * - `edit`：若文档包含模板字段，则固定内容锁定、仅可填写字段；隐藏工具栏。
   * - `readonly`：等同 `readOnly: true`，内容不可编辑。
   * - `design`：可自由编辑内容，并可将块标记为「固定 / 可填写」（模板设计，显示工具栏）。
   * 当 `mode` 为 `readonly` 时，`readOnly` 会被忽略。
   */
  mode?: EditorMode;
}
...
/**
 * 将 React 层的 mode 同步到模板字段扩展的 SET_TEMPLATE_MODE_COMMAND，
 * 使 lock-filter 在 edit（填写）模式下生效。
 */
...
export function Editor({
  ...
  const mode: EditorMode = modeProp ?? (readOnlyProp ? 'readonly' : 'edit');
  ...
  // 工具栏显示策略：
  // - design：显示（插入字段、配置字段依赖工具栏）。
  // - edit：模板文档隐藏工具栏（填写模式不提供结构/格式修改入口），
  //   普通文档保持工具栏可见。
  ...
  // 初始可编辑状态仅在首次挂载时生效,后续切换由 ReadOnlySync 控制
  const initialReadOnlyRef = useRef(!editable);
    ...
    () =>
      defineExtension({
        ...
        dependencies: [
          ...
          configExtension(InitialValueExtension, {
            initialValue: parsedInitialValue,
          }),
          configExtension(OnChangeExtension, {
            onChangeRef: onChangeWrapperRef,
          }),
          InsertImageExtension,
          InsertVideoExtension,
          InsertAudioExtension,
