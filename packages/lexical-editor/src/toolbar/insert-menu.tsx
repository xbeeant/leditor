...
import { INSERT_TEMPLATE_FIELD_COMMAND } from '../plugins';
...
function getRightSections(
  ...
  const isDesign = config?.mode === 'design';
  ...
  if (isDesign) {
    advancedItems.push({
      type: 'templateField',
      label: t(locale, 'insertTemplateField'),
    ...
    (type: InsertBlockType) => {
      switch (type) {
        ...
        case 'mind':
          ...
          break;
        case 'templateField':
          editor.dispatchCommand(INSERT_TEMPLATE_FIELD_COMMAND, undefined);
          closeMenu();
          break;
        default:
          break;
