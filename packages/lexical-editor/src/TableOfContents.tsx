...
function $getHeadingList(): TocItem[] {
  const root = $getRoot();
  const items: TocItem[] = [];
  ...
  const visit = (node: LexicalNode) => {
    if ($isHeadingNode(node)) {
      ...
      items.push({
        key: node.getKey(),
        text: node.getTextContent().trim() || '(untitled)',
        level,
      });
      return;
    }
    ...
    return (
      <aside className="flex w-60 shrink-0 flex-col border-l border-gray-200 bg-gray-50/50 p-5">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-gray-900">Contents</span>
        </div>
        <div className="-mx-1 flex-1 overflow-y-auto px-1 pb-1 pt-2">
          {items.length === 0 ? (
            <p className="text-sm text-gray-400">No headings yet</p>
          ) : (
            list
          )}
        </div>
