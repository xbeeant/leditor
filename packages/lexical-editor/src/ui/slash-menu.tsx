...
import { useEditorConfig, useLocale } from '../context';
import { t } from '../i18n';
...
export function SlashMenu({
  ...
  const hasMediaConfig = !!editorConfig?.embed?.attachment?.action;
  const filtered = SLASH_ITEMS.filter((item) => {
    if (!hasMediaConfig && (item.action === 'image' || item.action === 'file')) {
      return false;
    }
    return item.keyword.some((kw) => kw.toLowerCase().includes(query.toLowerCase()));
  });
